...
func findCleanAgents() -> [String] {
    ...
    for agent in agentArray {
        ...
        if agent.compromised == false {
            ...
            let agentInfo: String = agent.realName + ", " + String(agent.accessLevel)
            cleanAgents.append(agentInfo)
        }
    }
    return cleanAgents
}
...
findCleanAgents()
print("\(cleanAgents.count) clean agents out of \(agentArray.count) total agents")
...
func agentRosterInfo() {
    var agentRoster = agentArray
    agentRoster.sort(b)
    for agent in agentRoster {
        var element = agent
        
        var count = 0 + agent.accessLevel
        agentRoster.
        print(agent.coverName + agent.realName + String(agent.accessLevel))
